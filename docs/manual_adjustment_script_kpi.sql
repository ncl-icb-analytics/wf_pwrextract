...
DELETE FROM [Data_Lab_NCL_Dev].[JakeK].[wf_pwr_kpi]
WHERE 
	fyear = @DestinationFYear 
	AND month = @DestinationFMonth
	AND org_code = @TargetOrgCode;

INSERT INTO [Data_Lab_NCL_Dev].[JakeK].[wf_pwr_kpi]
SELECT
	@DestinationFYear AS [fyear], 
	[org_code], [occ], [section], [subcode], [unit],
	@DestinationFMonth AS [month], 
	[count]
FROM [Data_Lab_NCL_Dev].[JakeK].[wf_pwr_kpi]
WHERE 
	fyear = @SourceFYear 
